...
import { API_BASE_URL } from "@/app/apis/url";
...
function getDeliveryDate(dayObj: any, timeId: string) {
  if (!dayObj || !timeId) return null;
  const [startHour] = timeId.split("-").map(Number);
  const d = new Date(dayObj.gDate);
...
export default function CheckoutPage() {
  ...
  async function handleCheckout() {
    if (!selectedDay || !selectedTime) {
      alert("روز و ساعت ارسال را انتخاب کنید");
      return;
    }

    const dayObj = availableDays.find((d) => d.id === selectedDay);
    const deliveryDate = getDeliveryDate(dayObj, selectedTime);
    ...
    console.log(order.data.order._id);

    const payRes = await fetch("/apis/payment", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        amountToman: finalTotal,
        orderId: order.data.order._id,
        description: `سفارش شماره ${order._id}`,
      }),
    });
    console.log(payRes);
    const payData = await payRes.json();
    if (!payRes.ok || !payData?.trackId) {
      console.error(payData);
      alert("خطا در اتصال به درگاه پرداخت");
      return;
    }

    window.location.href = `https://gateway.zibal.ir/start/${payData.trackId}`;
  }
      ...
      <div className="max-w-7xl mx-auto p-4 grid grid-cols-1 lg:grid-cols-4 gap-6 text-black">
        ...
        <div className="lg:col-span-1 text-natural-900">
          <Card className="p-4">
            ...
            <Button
              className="w-full mt-6 bg-primary hover:bg-primary/80 text-white"
